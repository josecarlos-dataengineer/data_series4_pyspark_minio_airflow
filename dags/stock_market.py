...
def stock_market():
    ...
    def is_api_available() -> PokeReturnValue:
        api = BaseHook.get_connection("stocks_yahoo")
    ...
    load_to_dw = aql.load_file(
        task_id="load_to_dw",
        input_file=File(path=f"s3://{BUCKET_NAME}/"+"{{task_instance.xcom_pull(task_ids='_get_formatted_csv')}}", conn_id="minio"),
        output_table=Table(
            name="stock_market",
            conn_id="postgres",
            metadata=Metadata(
